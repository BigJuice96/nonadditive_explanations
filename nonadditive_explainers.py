...
def Omega(X, feature_index, q_additivity= None, feature_type='numerical'):
    n, d = X.shape
    ...
    idx = np.arange(d)
    idx[feature_index], idx[0] = idx[0], idx[feature_index]
...
# THIS IS THE NEW ONE GPT SUGGESTED
def Omega(X, feature_index, q_additivity=None, feature_type='numerical'):
    n, d = X.shape
    if q_additivity is None:
        q_additivity = d
    ...
    idx = np.arange(d)
    # Swap the target feature (feature_index) into the first position.
    idx[0], idx[feature_index] = idx[feature_index], idx[0]
    X = X[:, idx]

    if feature_type == 'binary':
        # Binary branch unchanged...
        # Use a different inner loop variable, e.g., k, instead of reusing feature_index.
        omega = np.zeros((n,))
        ind_nonzeros = np.where(X[:,0] > 0)[0].tolist()
        for k in ind_nonzeros:
            xi_ones = np.where(X[k, 1:] > 0)[0].tolist()
            xi_ones_count = len(xi_ones)
            temp = 0
            for j in range(1, q_additivity):
                temp += (1 / (j+1)) * (math.comb(xi_ones_count, j))
    ...
    # Use a different variable name in the loop (j here) to avoid conflict
    for j in range(d):
        dp[0, j, :] = X[:, j]
        sum_current += X[:, j]

    for level in range(1, q_additivity):
        temp_sum = np.zeros((n,))
        for j in range(d):
            sum_current -= dp[level - 1, j, :]
            dp[level, j, :] = (level / (level+1)) * X[:, j] * sum_current
            temp_sum += dp[level, j, :]
        sum_current = temp_sum
...
def nonadditive_explainer(data_x, y, cat_features, method = "SurvMLeX", alpha=1.0, finetune=True, rank_ratio=1.0, fit_intercept=False, n_shuffle_splits=5, random_state=None, range_alpha= (2.0 ** np.arange(-8, 7, 2))):
    """
